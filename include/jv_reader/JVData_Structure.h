...
typedef struct
{
		...
		char		BreederCode[6];           //生産者コード
		char		BreederName[70];          //生産者名
...
typedef struct
{
		_RECORD_ID	head;                     //<レコードヘッダー>
		char		BreederCode[6];           //生産者コード
		char		BreederName_Co[70];       //生産者名(法人格有)
		char		BreederName[70];          //生産者名(法人格無)
		char		BreederNameKana[70];      //生産者名半角カナ
...
typedef struct
{
		...
		char		SanchiName[20];           //産地名
		char		HansyokuFNum[8];          //父馬繁殖登録番号
		char		HansyokuMNum[8];          //母馬繁殖登録番号
		char		crlf[2];                  //レコード区切り
}				JV_HN_HANSYOKU;
...
typedef struct
{
		...
		char		BreederCode[6];           //生産者コード
		char		SanchiName[20];           //産地名
		char		HansyokuNum[14][8];       //3代血統 繁殖登録番号
		char		crlf[2];                  //レコード区切り
}				JV_SK_SANKU;
